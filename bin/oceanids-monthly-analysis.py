...
th2=63.5
df_max=df_all[df_all.percent_over_threshold >=th2] 
df_max_copy=df_max.copy()
df_max_copy.index = pd.to_datetime(df_max_copy.index)
dfg_ecxsf = df_max_copy.groupby(df_max_copy.index.month).size().reset_index(name='counts')
dfg_ecxsf['utctime'] = dfg_ecxsf['utctime'].map(dict(zip(range(1, 13), calendar.month_name[1:])))

df_obs_max=df_obs[df_obs.WG_PT1H_MAX>=th] 
df_obs_max=df_obs_max.set_index('utctime')
df_obs_max_copy=df_obs_max.copy()
df_obs_max_copy.index = pd.to_datetime(df_obs_max_copy.index)
dfg_obs = df_obs_max_copy.groupby(df_obs_max_copy.index.month).size().reset_index(name='counts')
...
over_th = df_ecsf.iloc[:, 1:] >= th
count_over_th = over_th.sum(axis=1)
df_ecsf['percent_over_threshold'] = (count_over_th / 51) * 100

df_max_sf=df_ecsf[df_ecsf.percent_over_threshold >=th2] 
#print(df_max_sf)
df_max_sf_copy=df_max_sf.copy()
df_max_sf_copy.index = pd.to_datetime(df_max_sf_copy.index)
dfg_ecsf = df_max_sf_copy.groupby(df_max_sf_copy.index.month).size().reset_index(name='counts')
