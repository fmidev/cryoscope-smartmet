...
#seq 0 50 | parallel --compress cdo -b P12 -O --eccodes remap,$grid-$abr-grid,ec-sf-$grid-$abr-weights.nc ens/ec-sf_$year${month}_swvls-24h-$abr-{}-fixLevs.grib ens/ec-sf_${grid}_$year${month}_swvls-24h-$abr-{}-fixLevs.grib || echo 'not remappig swvls - already done or no input file'
...
seq 0 50 | parallel --compress grib_copy -w shortName=2t/2d/stl1/rsn/sde ens/ec-BSF_${year}${month}_unbound-24h-eu-{}-fixed.grib ens/ec-BSF_${year}${month}_snow-24h-eu-{}-fixed.grib  \
...
echo 'netcdf to grib'
# netcdf to grib
seq 0 50 | parallel --compress cdo -b 16 -f grb2 copy -setparam,41.228.192 -setmissval,-9.e38 -seltimestep,9/209 ens/ECXSF_${year}${month}_swi2_${grid}_out-{}.nc ens/ECXSF_${year}${month}_swi2_${grid}_out-{}.grib #|| echo "NO input or already netcdf to grib1"

echo 'grib fix'
# fix grib attributes
seq 0 50 | parallel --compress grib_set -r -s centre=86,productDefinitionTemplateNumber=1,totalNumber=51,number={} ens/ECXSF_${year}${month}_swi2_${grid}_out-{}.grib \
